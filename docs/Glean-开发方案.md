...
> 文档版本：0.2.1 · 日期：2026-07-28  
> 修订说明：吸收架构评审；**已拍板：优先纯 Rust 壳（路径 A），M0 Spike 不过再切 Tauri**
...
| M0 UI Spike | 见 **§9.0 量化清单**；未通过则切换 UI 栈后再写业务 |
...
**下一步（唯一）：** 启动 **M0 UI Spike**——egui 三栏 + WebView 单实例，按 §9.0.2 打表；结论写入 `docs/spike-ui.md`。未 Pass 之前不写订阅业务。
