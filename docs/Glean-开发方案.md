...
> 文档版本：0.5.5 · 日期：2026-08-04  
> 修订说明：**M0–M5 主线全部落地**（订阅/阅读/组织/搜索/离线/打磨/插件框架，138 测试绿）；**M6 大部分完成**（Pixiv 适配器、AI 增强可读展示、DPAPI/keyring 凭证加密）；开发过程中追加的临时需求（UI/数据可靠性/刷新限流/停止刷新/AI 展示修复/抽取走代理等）已整理进 **§13**。  
...
| 34 | AI 结果追加在正文末尾不可见 | 移到正文**上方**（正文可能很长，底部结果滚动不到）；新增 `reader_document` 回归测试 | ✅ 已实现 |
| 35 | AI 失败反馈不明显 | Failed 改发 `AppEvent::Error`（进入错误日志弹窗 + 状态栏「错误:」前缀） | ✅ 已实现 |
| 36 | AI 结果被图片缓存重渲染覆盖 | `poll_img_cache` 用不含 enhancements 的 body 重渲染并 `show_html`，覆盖刚显示的摘要/翻译；抽取 `with_enhancements()` 在图片缓存重渲染时复用（Pixiv 等含防盗链图的文章必现） | ✅ 已实现 |
| 37 | 抽取全文不走代理、同步阻塞 UI | `maybe_auto_extract` / `extract_current` 改为异步 + 优先走代理 client（与图片缓存一致）；Pixiv 作品页自动跳过（插件已提供图文，网页需登录无正文可读） | ✅ 已实现 |
| 38 | 已缓存文章无法重新抽取 | `prepare_extract_task(force=true)` 允许重抽已抽取内容；手动「抽取全文」按钮不再静默跳过 | ✅ 已实现 |
| 39 | Pixiv meta 缺发帖时间 | 新增 `unix_to_date` host 函数（chrono），适配器 meta 区加 📅 日期显示 | ✅ 已实现 |
